...
### Fetching Programs
```javascript
const response = await fetch('https://stackenzo-backend.onrender.com/api/programs');
...
### Filtering Programs
```javascript
const response = await fetch('https://stackenzo-backend.onrender.com/api/programs?type=workshop&status=upcoming');
```
...
```bash
curl -X POST https://stackenzo-backend.onrender.com/api/programs \
...
### Test Registration
```bash
curl -X POST https://stackenzo-backend.onrender.com/api/programs/register \
  -H "Content-Type: application/json" \
  -d '{
    "programId": 1,
    "name": "Test User",
