...
function ResumeAdmin() {
  ...
  const fetchResumes = async () => {
    try {
      const url = statusFilter 
        ? `https://stackenzo-backend.onrender.com/api/resumes?status=${statusFilter}`
        : "https://stackenzo-backend.onrender.com/api/resumes";
  ...
  const viewResume = (id) => {
    window.open(`https://stackenzo-backend.onrender.com/api/resumes/${id}/view`, '_blank');
    // window.open(`http://localhost:3000api/resumes/${id}/view`, '_blank');
  };

  const downloadResume = async (id, filename) => {
    try {
      const response = await fetch(`https://stackenzo-backend.onrender.com/api/resumes/${id}/download`);
  ...
  const updateStatus = async (id, newStatus) => {
    try {
      const response = await fetch(`https://stackenzo-backend.onrender.com/api/resumes/${id}/status`, {
        method: "PATCH",
        headers: {
